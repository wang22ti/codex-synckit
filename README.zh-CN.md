...
| 会话历史 | 关闭 | 让另一台电脑重新打开并继续同一个 Codex 任务；其中可能包含完整提示词和回答 |
| 侧边栏与项目组织 | 关闭 | 同步任务标题、置顶、项目分组、排序和工作区提示 |
| 凭据、审批规则、缓存、日志、本地偏好、`state_5.sqlite` | 永不同步 | 为了安全和稳定，这些内容始终保留在本机 |

“会话历史”和“侧边栏与项目组织”就是涉及私有数据的功能。它们只服务于一个
具体需求：换到另一台电脑后，继续原来的 Codex 任务，并看到相同的任务组织。
由于其中可能包含对话正文、项目名称和本地路径，所以默认关闭。启用前请阅读
...
应用推荐链接，并创建托管的 `ChatGPT` 快捷方式。除非明确启用，否则会话和桌面
状态同步仍保持关闭。
